...
export const NewMoviesItemChange = ({ id, ratingSum, ratingCount, img, price, name }) => {
  const [isFocus, setIsFocus] = useState(false)
  return (
    <div onMouseEnter={() => setIsFocus(true)}
     onMouseLeave={() => setIsFocus(false)}
     >
      {isFocus ? (
        <NewMoviesItemFocus id={id} ratingSum = {ratingSum} ratingCount={ratingCount} img={img} price={price} name={name} setIsFocus={setIsFocus} />
      ) : (
