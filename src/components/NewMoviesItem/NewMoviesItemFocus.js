...
export default function NewMoviesItemFocus({ id, img, ratingSum, ratingCount }) {
  const [modalShow, setModalShow] = useState(false)
  // console.log('right sum = ... ',ratingSum);
  // console.log('right count = ... ',ratingCount);

  const rating = ratingCount>0 ? ratingSum/ratingCount : 0
  // console.log('right rating = ... ',rating);
  ...
  return (
    ...
    >
      ...
      <PiBookmarkSimpleBold
        ...
        style={{ position: 'absolute', top: '20px', right: '15px', transform: 'scale(1.6)' }}
      />
      <FaRegStar
        onClick={() => setModalShow(true)}
        title='Уже смотрел, оценить'
        style={{ position: 'absolute', top: '60px', right: '15px', transform: 'scale(1.6)' }}
      />
