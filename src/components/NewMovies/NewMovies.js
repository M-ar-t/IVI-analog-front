...
import { selectNewMovies } from '../../store/filmsReducer'
  ...
  return (
    <Container className='mt-5 pt-3 text-bg-dark'>
      ...
      <Container className='d-flex gap-4 pb-5'>
        {newMovies.map((item) => (
          <NewMoviesItemChange rating={item.rating} key={item.id} img={item.img} id={item.id} name={item.name} price={item.price} />
        ))}
      </Container>
    </Container>
  )
